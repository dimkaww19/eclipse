...
aimBtn.MouseButton1Click:Connect(function()
    if S.aimMode == "toggle" then
        ...
        updateAimBtnVisual()
        showNotification("AIM", S._aimHeld and "ON" or "OFF", S._aimHeld and Color3.fromRGB(0,255,100) or Color3.fromRGB(255,50,50))
    end
end)
...
-- Aimbot (mobile: CFrame override after camera, no Scriptable switch)
S._aimTarget = nil
RS:BindToRenderStep("EclipseAimbot", Enum.RenderPriority.Camera.Value + 1, function()
    ...
    if not cfg.Enabled then S.lockedTarget = nil S._aimTarget = nil return end
    if not S._aimHeld then S.lockedTarget = nil S._aimTarget = nil return end
...
-- Second pass: apply camera after everything
RS:BindToRenderStep("EclipseAimApply", Enum.RenderPriority.Last, function()
    if S._aimTarget then
        local camPos = Camera.CFrame.Position
        Camera.CFrame = CFrame.new(camPos, S._aimTarget)
    end
end)
