...
floatBtn.BackgroundColor3 = Color3.fromRGB(168, 85, 247)
floatBtn.Text = "E"
...
floatStroke.Color = Color3.fromRGB(236, 72, 153)
...
do
    ...
    tgLabel.Size = UDim2.new(0, 150, 1, 0)
    tgLabel.Position = UDim2.new(0, 100, 0, 0)
    tgLabel.BackgroundTransparency = 1
    tgLabel.Text = "Mobile"
    tgLabel.TextColor3 = Color3.fromRGB(168, 85, 247)
...
do
    ...
    -- Car ESP
    local ceRow = makeRow(vp); vpNext(ceRow)
    local ceDot = makeDot(ceRow)
    makeLabel(ceRow, nil, "esp_cars")
    local ceBg, ceBall = makeToggle(ceRow)
    ceBg.MouseButton1Click:Connect(function()
        S.carESP = not S.carESP
        animateToggle(ceBg, ceBall, ceDot, S.carESP)
        if not S.carESP then
            for hl, _ in pairs(S.carESPCache) do pcall(function() hl:Destroy() end) end
            S.carESPCache = {}
        end
    end)
...
do
    ...
    local function doFarmCycle()
        ...
        local sp = sellPart:FindFirstChild("SellSmuggledGoodsPrompt")
        for attempt = 1, 5 do
            if not S.autoFarm then return false end
            firePromptSafe(sp)
            task.wait(1)
...
do
    ...
    table.insert(aimSubs, makeSlider(ap, nil, 20, 350, 200, Color3.fromRGB(168,85,247), function(v)
        if v > 300 then _G.AimbotConfig.MaxDistance = 9999 else _G.AimbotConfig.MaxDistance = v end
    ...
    if IsBindPressed() then
        ...
        if S.lockedTarget and S.lockedTarget.Character then
            ...
            if aPart then
                ...
                pcall(function() mousemoverel(dx * cfg.Smoothness, dy * cfg.Smoothness) end)
            end
        end
    else
        S.lockedTarget = nil
            ...
            for _, obj in ipairs(WS:GetDescendants()) do
                if obj:IsA("VehicleSeat") then
                    ...
                    if veh and veh ~= LocalPlayer.Character and not found[veh] then
                        ...
                        if partCount >= 8 or hasWheel then found[veh] = true end
                    end
                end
            end
