            ...
            for _, pl in ipairs(Players:GetPlayers()) do
                if pl ~= LocalPlayer and pl.Character then
                    ...
                    if hum and hum.Health > 0 then
                        local sameTeam = (cfg.TeamCheck and pl.Team and LocalPlayer.Team and pl.Team == LocalPlayer.Team)
                        if not sameTeam then count = count + 1 end
                    end
                end
            end
            showNotification("AIM DEBUG", "Targets: "..count.." | FOV: "..cfg.FOV.." | Dist: "..cfg.MaxDistance, Color3.fromRGB(255,255,0))
        else
            showNotification("AIM", "OFF", Color3.fromRGB(255,50,50))
        end
    end
end)
...
-- Aimbot targeting + camera (separate thread, runs after camera update)
S._aimThread = task.spawn(function()
    while S.scriptActive do
        task.wait()
        local cfg = _G.AimbotConfig
        if not cfg or not cfg.IsRunning or not cfg.Enabled or not S._aimHeld then
            S.lockedTarget = nil
            pcall(function()
                    ...
                    if isValidTarget(pl) then
                        ...
                        if aPart then
                            ...
                            if dist3d <= cfg.MaxDistance then
                                if not cfg.WallCheck or isVisible(aPart) then
...
do
    ...
    table.insert(S.connections, RS.RenderStepped:Connect(function()
        if not S.scriptActive or not S.antiRecoilEnabled then S.arPrevPitch = nil return end
